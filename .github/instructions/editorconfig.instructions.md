...
3. Run `dotnet format` and `dotnet pprettier` to verify changes
...
- [ ] `dotnet format Qwiq.sln --verify-no-changes` passes (or changes are intentional)
- [ ] `dotnet pprettier` passes with no formatting issues
